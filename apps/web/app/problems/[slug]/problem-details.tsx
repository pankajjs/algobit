...
export const ProblemDetails = ({problem}:{problem:any}) => {
    ...
    return  <Suspense fallback={"Laoding....."}>
        <div className="min-w-[50%]">
            <div className="border border-[#383839]">
                ...
                <Button className={"text-gray-100 " + (showSubmissions && "underline")} variant={"link"} onClick={handleSubmissionsClick}
                >Submissions</Button>
            </div>
        ...
        {showSubmissions && <div className="overflow-y-scroll h-full">
            {submissionList.map((submission:any, idx:number)=>{
                ...
                return <div key={submission.id + idx}><div onClick={()=>{
                    if(showSubmissionDetails && submissionDetails.id === submission.id){
                        setShowSubmissionDetails(false)
                        return;
                    }
                    setShowSubmissionDetails(true)
                    setSubmissionDetails(submission);
                }}  className={"text-white p-2 border border-[#383839] hover:border-[#66666a] hover:cursor-pointer"}>
                <div className="flex w-max items-center gap-20 justify-between">
                        <div className="min-w-max">
                            <div className={(submission.status === "Success"?"text-green-400":"text-red-500") + " text-sm"}>{submission.status}</div>
                ...
                {showSubmissionDetails && submissionDetails && submissionDetails.id === submission.id &&  <div className="flex flex-col p-5">
                            <pre className="border border-gray-500 text-white p-2 text-xs rounded-md bg-[#2f2f30] text-wrap">{submissionDetails.code}</pre>
