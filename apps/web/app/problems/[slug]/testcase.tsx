...
export const Test = ({testCases}:{testCases:any})=>{
    ...
    return <div className="">
        <div className="border border-[#383839]">
            <Button className={(showTestInput && "underline") + " text-gray-300"} variant={"link"} onClick={()=>{
                setShowTestResult(false);
                setShowTestInput(true);
            }}>Test Input</Button>
            <Button className={(showTestResult && 'underline') + " text-slate-300"} variant={"link"} onClick={
                ()=>{
                    setShowTestInput(false);
                    setShowTestResult(true);
                }
        ...
        {showTestResult && testResult && <div>
            
            {testResult.status === "Error" && <div className="text-red-600">
                <div className="border-b-2 border-[#383839] p-2">{testResult.status}</div>
            ...
            <div className="flex flex-col">
                <div className={(testResult.status === "WA" ? "text-red-700": "text-green-700") + " border-b-2 border-[#383839] p-2"}>{testResult.status ==="WA"?"Wrong Answer":testResult.status}</div>
                <div className="flex gap-4 p-2">
                    {
                    testResult.input.map((_:string, idx:number)=>{
                        return <Button key={idx} onClick={()=>{
                            setTestCase(idx+ 1)
                        }} variant={"secondary"}>Test #{idx+1}</Button>
                    })
                }
                </div>
                <div className="">
                    ...
                    return <div key={idx} className="px-2">
                        {testCase === idx + 1 && <div>
                            ...
                            <pre className={(testResult.status === "WA" ? "text-red-700": "text-green-700") + " border-gray-700 border rounded-md px-4 py-2"}>
                            ...
                            <Label className="text-slate-50">Expected Output</Label>
                            <pre className={(testResult.status === "WA" ? "text-red-700": "text-green-700") + " border-gray-700 border rounded-md px-4 py-2"}>
