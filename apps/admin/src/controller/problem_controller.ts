import type { IProblemService } from "@repo/types";
...
export default class ProblemController {
	...
	async deleteProblem(req: Request, res: Response, next: NextFunction) {
		try {
			...
			logger.error(`Error while deleting a problem: ${error}`);
			next(error);
		}
	}
	...
	async updateProblem(req: Request, res: Response, next: NextFunction) {
		try {
			...
			logger.error(`Error while updating a problem: ${error}`);
			next(error);
		}
	}
	...
	async createProblem(req: Request, res: Response, next: NextFunction) {
		try {
			...
			logger.error(`Error while creating a problem: ${error}`);
			next(error);
		}
	}
	...
	async getAllProblem(_: Request, res: Response, next: NextFunction) {
		try {
			...
			logger.error(`Error while fetching all problems: ${error}`);
			next(error);
		}
	}
	...
	async getProblemByName(req: Request, res: Response, next: NextFunction) {
		try {
			...
			logger.error(`Error while fetching problem by name: ${error}`);
			next(error);
		}
	}
}
