...
app.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			if (!socketId) {
				throw new NotFoundError(`User not connected for userId:${id}`);
			}
			...
			if (!socket) {
				throw new NotFoundError(
					`Socket connection not found for socketId:${socketId}`,
				);
			}
			...
			logger.error(
				`Error while sending response to web socket client:${error}`,
			);
			next(error);
		}
	},
